...
Game.prototype.inspect = function(room, response){
  if(!room[response]) {
    ...
    if(room[response]['actions']['inspect']) {
      ...
      console.log('You cannot inspect that item.');
    }
  }
};
...
Game.prototype.passcode= function(room, response){
   if(room.alias != 'Giraffe Encounter' && room.alias != 'The Rainforest Room') {
    ...
    } else if (response == '54321' && room.alias == 'The Rainforest Room') {
